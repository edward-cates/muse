...
export function readElement(m: Y.Map<YMapVal>): CanvasElement {
  ...
  if (type === 'text') {
    return {
      ...
      fontSize: (m.get('fontSize') as number) || 20,
      fontFamily: (m.get('fontFamily') as string) || 'system-ui, -apple-system, sans-serif',
...
export function useElements() {
  ...
  const addText = useCallback(
    (x: number, y: number): string => {
      ...
      yEl.set('text', '')
      yEl.set('fontSize', 20)
      yEl.set('fontFamily', 'system-ui, -apple-system, sans-serif')
